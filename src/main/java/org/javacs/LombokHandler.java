...
public final class LombokHandler {
    ...
    public static void addCompletions(
        ...
        // Add Slf4j log variable
        if (metadata.hasSlf4j && !existing.contains("log") && StringSearch.matchesPartialName("log", partial)) {
            var item = new CompletionItem();
            item.label = "log";
        ...
        while (path != null) {
            if (path.getLeaf().getKind() == Tree.Kind.CLASS) {
                ...
                if (enclosingElement instanceof TypeElement) {
                    addCompletions(task, (TypeElement) enclosingElement, partial, list, cache);
                }
                return;
            }
